...
**模型可见 ⟺ 已持久引用。** 凡到达模型请求的内容都必须能从会话日志及其引用的不可变内容寻址对象中重建。可检查的推论：任何人持有日志、日志引用的附件对象和固定代码版本，即可逐字节重建循环的每个请求。纯文本 `GenerateOptions` 仍是日志的纯函数；含图片请求还会在适配器序列化期间通过 `ctx.attachments` 解析 `ImageAttachmentRef` 字节，其中对内容摘要及已记录元数据的校验使对象查找具有确定性，并在数据缺失或损坏时明确失败。直接的一次性调用（压缩的 summarize 调用）记录其信封标量（`compaction/summary.{provider, model, maxTokens}`），其输入是对日志区域及这些引用对象的确定性代码运算——由于只有循环会标记请求归属，因此它们不在不变式内。该不变式约束的是循环的 `GenerateOptions` 及其 `EpochHeader`；适配器随后序列化出的提供方专属请求体还会加入在派发时解析的字段——合并进来的扩展字段、Files API 文件 id，以及最终采用的图片表示形式——对于这些字段，[请求线上报文捕获决策](2026-09-25-durable-request-wire-capture.zh.md)选择单独记录而非推导。
